# with all the code same as above, but we will replace two lines marked with star (*)
...
for i in range(1, r-1):
    if(prefix_row_sum[i] == prefix_row_sum[i-1]):
        dis_sum += prefix_row_sum[i]*(prefix_row_sum[r-1]-prefix_row_sum[i])*(1000000-1) # ------------> (*)

for j in range(1, c-1):
    if(prefix_col_sum[j] == prefix_col_sum[j-1]):
        dis_sum += prefix_col_sum[j]*(prefix_col_sum[c-1]-prefix_col_sum[j])*(1000000-1) # ------------> (*)
